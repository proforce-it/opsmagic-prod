                                ...
                                <div class="alert-text text-center">
                                    <div class="row">
                                        <div class="col-lg-12 text-center d-flex justify-content-center align-items-center">
                                            <div class="bg-secondary p-5 me-1 fs-3" style="border-top-left-radius: 10px; border-bottom-left-radius: 10px;">
                                                <div>
                                                    <a href="javascript:;" id="previous_wa_week"><i class="las la-arrow-circle-left fs-2 text-dark"></i></a>
                                                    w/c <span id="worker_availability_tab_date"></span>
...
@section('worker_availability_js')
    <script>
        ...
        let worker_availability_table = tableNameWorkerAvailabilityDatatable.DataTable();

        $('#worker_availability').on('click', function () {
            worker_availability_table.destroy()
            worker_availability_table = tableNameWorkerAvailabilityDatatable.DataTable({
                    ...
                    "dataSrc": function (json) {
                        if (json.table_th) {
                            for (let i = 0; i < 7; i++) {
                                ...
                                $('#day_' + (i + 1) + '_th').html(`${day.title}`);

                                const headerHtml2 = `${day.shiftTrueOrFalse
                                    ? `<div class="mt-1 small fw-bold">
                                            ${day.confirm}c / ${day.invited}I / ${day.available}A
                                        </div>`
                                    : ``
                                }`;
                                $('#day_' + (i + 1) + '_th2').html(headerHtml2);
                            }
                        }
                        return json.data;
                    }
                ...
                "columns": [
                    {"data": "worker_detail", "sClass":"position-relative p-0", "width":"16%"},
                    {"data": "day_1", "sClass":"position-relative p-0", "width":"12%"},
                    {"data": "day_2", "sClass":"position-relative p-0", "width":"12%"},
                    {"data": "day_3", "sClass":"position-relative p-0", "width":"12%"},
                    {"data": "day_4", "sClass":"position-relative p-0", "width":"12%"},
                    {"data": "day_5", "sClass":"position-relative p-0", "width":"12%"},
                    {"data": "day_6", "sClass":"position-relative p-0", "width":"12%"},
                    {"data": "day_7", "sClass":"position-relative p-0", "width":"12%"},
                ],
